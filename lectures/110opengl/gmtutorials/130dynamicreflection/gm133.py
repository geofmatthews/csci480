...
# Do NOT render the relfector itself to its own cameras.
...
def display(time):
    ...
    # draw the scene
    width,height = theScreen.get_size()
    displayFB(time, 0, width, height, theCamera, showMesh=True)

# Here we parameterize the display routine with a framebuffer
# and a camera
def displayFB(time, framebuffer, width, height, camera, showMesh=True):
