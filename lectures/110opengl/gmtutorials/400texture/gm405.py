...
# NO lighting calculations, but perspective on textures
# creates some depth perception
...
def readShader(filename):
    with open(os.path.join("..","shaders", filename)) as fp:
        return fp.read()
strVertexShader = readShader("flattextured.vert")
strFragmentShader = readShader("flattextured.frag")
...
def display(time):
    ...
    glUniform2fv(scaleuvUnif, 1, N.array((1.0,1.0), dtype=N.float32))
