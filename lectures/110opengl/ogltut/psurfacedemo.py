...
def main ():
    ...
    programID = loadProgram(
        os.path.join("shaders","psurfacedemo.vert"),
        os.path.join("shaders","psurfacedemo.frag")
        )
    ...
    vertNormUVData, vertNormUVIndices = saddle(N.arange(-1,1,.1),
                                               N.arange(-1,1,.1))
    vertNormUVData, vertNormUVIndices = arrow()
    ...
    while running:
        ...
        #ld = N.dot(rotationYMatrix(time/2), lightDirection);
        ld = lightDirection
        ...
        if solid:
            ...
            mode = GL_LINES
        glDrawElements(mode, numberOfVertices*bytesPerShort,
