#version 330

// Input vertex buffer data
// Each execution of this shader gets one vertex from the buffer
in vec4 positionBuffer;
in vec4 normalBuffer;
in vec4 tangentBuffer;
in vec4 bitangentBuffer;
in vec2 uvBuffer;

// Uniforms
// All executions of this shader get the same value for these
uniform mat4 M; // model
uniform mat4 V; // view
uniform mat4 P; // projection
uniform vec4 lightDirection;

// Values interpolated for fragment shader
out vec4 position;
out vec4 positionWorld;
out vec4 positionCamera;
out vec4 normal;
out vec4 tangent;
out vec4 bitangent;
out vec4 light;
out vec4 eye;
out vec2 uv;

void main(){
    uv = uvBuffer;
    // don't bother normalizing, we have to renormalize in
    // the fragment shader anyway
    light = V * lightDirection;
    normal =  V * M * normalBuffer;
    tangent = V * M * tangentBuffer;
    bitangent = V * M * bitangentBuffer;
    position = positionBuffer;
    positionWorld = M * position;
    positionWorld.x += gl_InstanceID;
    positionWorld.y -= gl_InstanceID;
    positionCamera = V * positionWorld;
    eye = -(V * M * position);
    gl_Position = P * positionCamera;
}

